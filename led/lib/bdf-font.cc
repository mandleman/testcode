  ...
  while (fgets(buffer, sizeof(buffer), f)) {
    if (sscanf(buffer, "FONTBOUNDINGBOX %d %d %d %d",
      ...
      *current_glyph = tmp;
      ...
      bitmap_shift =
        8 * (sizeof(rowbitmap_t) - ((current_glyph->width + 7) / 8))-tmp.x_offset;
         current_glyph->width+=tmp.x_offset;
      row = -1;  // let's not start yet, wait for BITMAP
    }
    else if (strncmp(buffer, "BITMAP", strlen("BITMAP")) == 0) {
      row = 0;
    }
    else if (sscanf(buffer,"DWIDTH %d %d",&tmp.dwidth,&dummy)==2){
    	if(current_glyph)current_glyph->dwidth=tmp.dwidth;
    }
