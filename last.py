...
class RunText(SampleBase):
    ...
    def run(self):
        ...
        middle_font = graphics.Font()
        middle_font.LoadFont("./fonts/test2_12.bdf")
        yellow_text = graphics.Color(255, 255, 0)
        green_text = graphics.Color(0, 255, 0)
        red_text = graphics.Color(255, 0,0)
        blue_text = graphics.Color(0, 0,255)
        ...
        while True:
            ...
            if int_now >= 6 and int_now <= 18:
                if int_now==18 and now_m>29:
                      ...
                      len = graphics.DrawText(offscreen_canvas, middle_font, 0, 0, red_text, "")
                ...
                while True:
                    ...
                    if int_now==18 and now_m>29:
                        break
                    if not (int_now >= 6 and int_now <= 18):
                        break
                    line=flag_text
                    if 'wait' in line:
                        offscreen_canvas.Clear()
                        ...
                        while True:
                            ...
                            if 'retry' in line2:
                                msg = '다시시도해주세요'
                                offscreen_canvas.Clear()
                                len = graphics.DrawText(offscreen_canvas, middle_font, pos+5, 20, yellow_text, msg)
                                offscreen_canvas = self.matrix.SwapOnVSync(offscreen_canvas)
                                time.sleep(3)
                            elif 'fail' in line2:
                                msg = '지정된카드가아닙니다.'
                                ...
                                print('recieved')
                                offscreen_canvas.Clear()
                                len = graphics.DrawText(offscreen_canvas, small_font, pos, 15, blue_text, msg)
                                len = graphics.DrawText(offscreen_canvas, small_font, pos+len+1, 15, red_text,carnum)
                                len1 = graphics.DrawText(offscreen_canvas, small_font, pos, 25, blue_text, daily)
                                len2 = graphics.DrawText(offscreen_canvas, small_font, pos+len1+2, 25, red_text, daily_num)
                                len3 = graphics.DrawText(offscreen_canvas, small_font, pos+len1+len2+5, 25, blue_text, monthly)
                                len4 = graphics.DrawText(offscreen_canvas, small_font, pos+len1+len2+len3+6, 25, red_text, monthly_num)
                                offscreen_canvas = self.matrix.SwapOnVSync(offscreen_canvas)
                                time.sleep(3)
                                break
